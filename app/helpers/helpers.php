...
if (!function_exists('validateMultiLang')) {
    /**
     * @param $rules
     * @return array
     */
    function validateMultiLang($rules)
    {
        $multiLangRules = [];
        foreach (locales() as $code => $locale) {
            foreach ($rules as $key => $value) {
                $multiLangRules[$locale . "." . $key] = $value;
            }
        }
        return $multiLangRules;
    }
}
...
if (!function_exists('str_limit')) {
     ...
     * @return bool
     */
    function str_limit($text, $limit = 20)
    {
